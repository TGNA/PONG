...
#define MAX_VEL 1.0
#define MAX_PALETA 9
#define MAX_THREAD MAX_PALETA+2
...
int cont=-1;
int numPaletes=0;
int numMoviments, numMovimentsMax;
...
void * mou_paleta_usuari(void * cap){
  do{
    ...
    pthread_mutex_unlock(&mutex_pantalla);
    if(numMoviments>0){
      if((tecla == TEC_AMUNT) || (tecla == TEC_AVALL))numMoviments--;
      pthread_mutex_unlock(&mutex_vglobals);
    }else{
      pthread_mutex_unlock(&mutex_vglobals);
      pthread_exit(0);
...
int main(int n_args, const char *ll_args[]){
  ...
  if (n_args == 4){
    numMoviments=atoi(ll_args[2]);
    numMovimentsMax=numMoviments;
  ...
  for(i=0; i<numPaletes; i++){
    pthread_create(&tid[2+i],NULL,mou_paleta_ordinador, (void *) (intptr_t) i);
  }

  inici=time(NULL);
  while((cont==-1)&&(numMoviments>0)&&(tecla != TEC_RETURN)){
    final=time(NULL);
    seg=difftime(final,inici);
    min=seg/60;
    seg=seg%60;
    pthread_mutex_lock(&mutex_vglobals);
    sprintf(buffer,"Fets: %d Restants: %d Temps: %2d:%2d", (numMovimentsMax-numMoviments),numMoviments, min, seg);
    pthread_mutex_unlock(&mutex_vglobals);
    pthread_mutex_lock(&mutex_pantalla);
    win_escristr(buffer);
    pthread_mutex_unlock(&mutex_pantalla);
  ...
  if (tecla == TEC_RETURN) printf("S'ha aturat el joc amb la tecla RETURN!\n");
    ...
    else if (cont > 0) printf("Ha guanyat l'usuari!\n"); 
